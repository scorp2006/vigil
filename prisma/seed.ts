...
async function main() {
  ...
  if (existing) {
    console.log("Demo org already exists. Deleting and recreating for a clean demo…");
  ...
  const employees: { id: string; category?: string }[] = [];
  const SEED_COUNT = 80;
  ...
  console.log(`${storedTemplates.length} templates seeded`);

  // Create 6 past campaigns across 90 days with realistic events
  const now = Date.now();
  for (let c = 0; c < 6; c++) {
  ...
  // Recompute risk scores for every employee
  ...
  console.log("\n✓ Seed complete.");
  console.log("  → Log in at /login with admin@acme.demo / demo1234");
}
